...
public class AppUtils {
    ...
    public static interface AppUtilsCallBack<O> {
        void result(O obj);
    }
    ...
    public static PackageInfo getInstallApp(Context context, String pkgName) {
        PackageManager pManager = context.getPackageManager();
        try {
            return pManager.getPackageInfo(pkgName, 0);
        } catch (PackageManager.NameNotFoundException e) {
            // TODO Auto-generated catch block
            return null;
        }
     ...
     * @param packageName
     */
    public static void getApkWithPackage(final Context context, final File dir, final String packageName, final AppUtilsCallBack<List<File>> callBack) {
        ...
        new AsyncTask<String, Void, List<File>>() {
            ...
            protected List<File> doInBackground(String... params) {
                ...
                for (File file : dir.listFiles()) {
                    ...
                    if (end.equals("apk")) {
                        //如果apk的包名和参数的包名一致的话
                        if (isApkPkgEqualPkg(context, file, packageName)) {
                            files.add(file);
                        }
                    }
                }
     ...
     * @return
     */
    public static boolean isApkPkgEqualPkg(Context context, File apkFile, String pkgName) {
     ...
     * @return
     */
    public static boolean isInstalledApk(Context context, File dir, String pkgName) {
        PackageManager pm = context.getPackageManager();
        if (!dir.exists() || !dir.isDirectory()) {
            return false;
        }
        File[] fileList = dir.listFiles();
        for (File string : fileList) {
            PackageInfo info = pm.getPackageArchiveInfo(string.getAbsolutePath(), PackageManager.GET_ACTIVITIES);
            if (pkgName != null && info != null) {
                if (info.packageName.equals(pkgName)) {
                    return true;
                }
